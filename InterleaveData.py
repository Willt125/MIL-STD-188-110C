...
def InterleaveChunk(inchunk: np.ndarray, numrows: int, numcols: int, rowInc: int, colDec: int) -> np.ndarray:
    ...
    if len(inchunk) != (numrows * numcols):
        raise RuntimeError(
            "The input chunk doesn't fit in the interleave matrix.")
    i = 0
    rownum = 0
    colnum = 0
...
def InterleaveData(indata: np.ndarray, Bd: int = 75, interleave_len: str = "S", frqmode: str = "fixed") -> np.ndarray:
    ...
    if (len(indata) % z) != 0:
        raise RuntimeError(
            "The input data does not evenly fit in the interleaver. Something is VERY wrong.")
